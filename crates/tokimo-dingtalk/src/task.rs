...
impl From<DtTask> for Task {
    fn from(t: DtTask) -> Self {
        Task {
            ...
            description: None,
            status: if t.done.unwrap_or(false) {
                TaskStatus::Done
            } else {
                TaskStatus::Pending
            },
            priority: dt_to_priority(t.priority.unwrap_or(20)),
            due_time: t.due_time.and_then(|s| {
...
impl TaskService for DingTalkClient {
    ...
    async fn update_task(&self, req: UpdateTaskRequest) -> ImResult<Task> {
        ...
        if let Some(ref title) = req.title {
            body.insert("subject".into(), serde_json::json!(title));
        }
        if let Some(ref p) = req.priority {
            body.insert("priority".into(), serde_json::json!(priority_to_dt(p)));
        }
        if let Some(ref s) = req.status {
            body.insert("done".into(), serde_json::json!(*s == TaskStatus::Done));
        }
        if let Some(ref due) = req.due_time {
            body.insert("dueTime".into(), serde_json::json!(due.to_rfc3339()));
        }
