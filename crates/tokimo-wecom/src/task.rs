...
impl TaskService for WeComClient {
    ...
    async fn list_tasks(&self, req: ListTasksRequest) -> ImResult<Page<Task>> {
        ...
        let detail_resp = self
            .post("/cgi-bin/oa/todo/get_detail", &detail_body)
            .await?;
    ...
    async fn update_task(&self, req: UpdateTaskRequest) -> ImResult<Task> {
        ...
        if let Some(ref title) = req.title {
            body.insert("content".into(), serde_json::json!(title));
        }
        ...
        if let Some(ref due) = req.due_time {
            body.insert(
                "remind_time".into(),
                serde_json::json!(due.format("%Y-%m-%d %H:%M:%S").to_string()),
            );
        }

        let resp = self
            .post("/cgi-bin/oa/todo/update", &serde_json::Value::Object(body))
            .await?;
