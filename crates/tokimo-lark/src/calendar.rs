...
impl CalendarService for LarkClient {
    async fn create_event(&self, req: CreateEventRequest) -> ImResult<CalendarEvent> {
        ...
        let resp = self
            .post("/open-apis/calendar/v4/calendars/primary/events", &body)
            .await?;
        let text = resp
            .text()
            .await
        ...
        let event = data
            .data
            .and_then(|d| d.event)
            .ok_or_else(|| ImError::Internal("no event".into()))?;
        Ok(event.into())
    }
    ...
    async fn list_events(&self, req: ListEventsRequest) -> ImResult<Page<CalendarEvent>> {
        ...
        let list = data.data.unwrap_or(ListEventData {
            items: vec![],
            page_token: None,
            has_more: None,
        });
    ...
    async fn update_event(&self, req: UpdateEventRequest) -> ImResult<CalendarEvent> {
        ...
        if let Some(ref st) = req.start_time {
            body.insert(
                "start_time".into(),
                serde_json::json!({"timestamp": st.timestamp().to_string()}),
            );
        }
        if let Some(ref et) = req.end_time {
            body.insert(
                "end_time".into(),
                serde_json::json!({"timestamp": et.timestamp().to_string()}),
            );
        }
        if let Some(ref l) = req.location {
            body.insert("location".into(), serde_json::json!({"name": l}));
        }
    ...
    async fn get_free_busy(&self, req: FreeBusyRequest) -> ImResult<Vec<BusySlot>> {
        ...
        let resp = self
            .post(
                "/open-apis/calendar/v4/calendars/primary/freebusy/query",
                &body,
            )
            .await?;
        let text = resp
        ...
        if let Some(arr) = val
            .get("data")
            .and_then(|d| d.get("freebusy_list"))
