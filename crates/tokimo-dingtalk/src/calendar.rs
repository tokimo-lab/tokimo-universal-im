...
use tokimo_core::{
    BusySlot, CalendarEvent, CalendarService, CreateEventRequest, FreeBusyRequest, ImError,
    ImResult, ListEventsRequest, Page, UpdateEventRequest,
};
...
impl CalendarService for DingTalkClient {
    async fn create_event(&self, req: CreateEventRequest) -> ImResult<CalendarEvent> {
        ...
        let resp = self
            .post("/v1.0/calendar/users/me/calendars/primary/events", &body)
            .await?;
    ...
    async fn update_event(&self, req: UpdateEventRequest) -> ImResult<CalendarEvent> {
        ...
        if let Some(ref st) = req.start_time {
            body.insert(
                "start".into(),
                serde_json::json!({"dateTime": st.to_rfc3339()}),
            );
        }
        if let Some(ref et) = req.end_time {
            body.insert(
                "end".into(),
                serde_json::json!({"dateTime": et.to_rfc3339()}),
            );
        }
    ...
    async fn get_free_busy(&self, req: FreeBusyRequest) -> ImResult<Vec<BusySlot>> {
        ...
        let resp = self
            .post(
                "/v1.0/calendar/users/me/calendars/primary/freeBusy/query",
                &body,
            )
            .await?;
        let status = resp.status();
